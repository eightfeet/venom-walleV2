import Modal, {
    createInlineStyles as inlineStyle,
    ModalParameters,
} from '@eightfeet/modal';
import Msg from './../Msg';
...
class AddressModal extends Modal {
    ...
    showModal = (submit, cancel, success) => {
        ...
        const playerDom = `
			...
			${
                this.playerPhone &&
                typeof this.checkVerificationCode === 'function'
    ...
    handleDom = (submit, cancel, success) => {
        ...
        if (this.cardIdRequest !== 1) {
            const noteDom: HTMLDivElement = modalElement.querySelector(
                `.${s.note}`
            );
            ...
            if (noteDom) {
                (modalElement.querySelector(`.${s.note}`) as any).onclick = (
                    e
                ) => {
                    e.stopPropagation();
    ...
    handleSubmit = (submit, success) => {
        ...
        const address = (modalElement.querySelector(`.${s.address}`) as any)
            .value;
